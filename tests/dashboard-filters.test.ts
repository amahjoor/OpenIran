...
test("getDashboardDateBounds returns custom and preset bounds", () => {
  assert.deepEqual(
    getDashboardDateBounds(
      { dateRange: "30d", customStart: "", customEnd: "", eventType: "all", country: "all" },
  ...
  assert.deepEqual(
    getDashboardDateBounds(
      { dateRange: "custom", customStart: "2026-03-01", customEnd: "2026-03-10", eventType: "all", country: "all" },
      new Date("2026-03-14T18:00:00Z"),
    ),
...
test("getAvailableCountries returns unique sorted country values", () => {
  ...
  assert.equal(
    describeDashboardDateRange(
      { dateRange: "ytd", customStart: "", customEnd: "", eventType: "all", country: "all" },
